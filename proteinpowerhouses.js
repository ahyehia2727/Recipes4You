...
app.post('/proteinpowerhouses', async (req, res) => {
  try {
    // Extract healthLabels from body. Expecting an array.
    const { healthLabels } = req.body;
    const dietCriteria = ["Low-Carb", "High-Protein"]; // Define diet criteria
    ...
    matchCriteria["recipe.dietLabels"] = { $all: dietCriteria };
