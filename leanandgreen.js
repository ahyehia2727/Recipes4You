...
// Route to get recipes that match the 'Vegan' health label
app.post('/leanandgreen', async (req, res) => {
  try {
    // Extract healthLabels from the body. Expecting an array.
    const healthLabels = req.body.healthLabels || [];
    
    // Ensure 'Vegan' is included in the healthLabels criteria
    if (!healthLabels.includes('Vegan')) {
      healthLabels.push('Vegan');
    }

    const matchCriteria = {
      "recipe.healthLabels": { $all: healthLabels }
    };

    const recipes = await Recipe.aggregate([
      { $match: matchCriteria },
      { $limit: 25 } // You can adjust the limit as needed
    ]);
